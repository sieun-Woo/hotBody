...
public class PostService {
    ...
    // 키워드로 게시글 검색
    public Page<PostResponseDto> searchPost(PostCategory postCategory, String searchType, String searchKeyword, GetPageModel getPageModel) {
        // 페이징 처리
        Pageable pageable = new PageDto().toPageable(getPageModel);
        ...
        if (postCategory == null) {
            if (searchType.equals("title")) {
                ...
                posts = postRepository.findByContentContaining(searchKeyword, pageable);
            } else {
                posts = postRepository.findByNicknameContaining(searchKeyword, pageable);
            }
        } else {
            if (searchType.equals("title")) {
                posts = postRepository.findByCategoryAndTitleContaining(postCategory, searchKeyword, pageable);
            } else if (searchType.equals("content")) {
                posts = postRepository.findByCategoryAndContentContaining(postCategory, searchKeyword, pageable);
            } else {
                posts = postRepository.findByCategoryAndNicknameContaining(postCategory, searchKeyword, pageable);
            }
        }
        return toPage(posts);
    }
    ...
    // 4. 게시글 수정
    @Transactional
    public ResponseEntity<String> updatePost(Long postId, PostModifyRequestDto postModifyRequestDto, User user) {
    ...
    public String modifyImage(Long postId, MultipartFile file) {
        ...
        Image image = uploadService.storeFile(file);
        String storeFileName = image.getStoreFileName();
        post.updateImage(storeFileName);
        return image.getStoreFileName();
    }

    // 7. 나의 게시글 조회
    @Transactional
    public Page<PostResponseDto> getMyAllPosts(String nickname, GetPageModel getPageModel) {
        // 페이징 처리
        Pageable pageable = new PageDto().toPageable(getPageModel);
    ...
    // 8. 키워드로 나의 게시글 검색
    @Transactional
    public Page<PostResponseDto> searchMyPosts(String nickname, String searchType, String searchKeyword, GetPageModel getPageModel) {
        // 페이징 처리
        Pageable pageable = new PageDto().toPageable(getPageModel);

        if (searchType.equals("title")) {
            Page<Post> posts = postRepository.findAllByNicknameContainingAndTitleContaining(nickname, searchKeyword, pageable);
            return posts.map(post -> new PostResponseDto(post));
        } else {
            Page<Post> posts = postRepository.findAllByNicknameContainingAndContentContaining(nickname, searchKeyword, pageable);
            return posts.map(post -> new PostResponseDto(post));
        }
    }
}
